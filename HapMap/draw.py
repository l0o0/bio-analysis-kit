...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        print "USAGE: python draw.py chrSample.txt region.txt out.svg"
        sys.exit(0)

    sample_data, region  = read_sample(sys.argv[1])
    dwg = svgwrite.Drawing(filename=sys.argv[3], size=(18 * cm, 13 * cm))
...
#    print Allpos
    ...
    dwg.add(dwg.text('%s:%s-%s, distance between blocks is converted in log10' % (region[0], min(Allpos), max(Allpos)), 
            insert=(0.1*cm, 11.5*cm), style="font-size:10px; font-family:Arial"))
